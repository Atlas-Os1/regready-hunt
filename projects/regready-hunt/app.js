...
let localLicenseScreenshot=false;let localLicenseObjectUrl=null;
document.querySelector('#license-image')?.addEventListener('change',event=>{const file=event.target.files?.[0];const preview=document.querySelector('#license-preview');const image=document.querySelector('#license-preview-image');if(localLicenseObjectUrl){URL.revokeObjectURL(localLicenseObjectUrl);localLicenseObjectUrl=null;}if(!file){localLicenseScreenshot=false;image.removeAttribute('src');preview.hidden=true;return;}if(!['image/png','image/jpeg','image/webp'].includes(file.type)||file.size>5*1024*1024){event.target.value='';localLicenseScreenshot=false;image.removeAttribute('src');preview.hidden=true;setAccountMessage('Choose a PNG, JPEG, or WebP image under 5 MB.');return;}localLicenseScreenshot=true;localLicenseObjectUrl=URL.createObjectURL(file);image.src=localLicenseObjectUrl;preview.hidden=false;setAccountMessage('Screenshot is previewed locally. Confirm only the fields you want to save.',true);});
document.querySelector('#license-form')?.addEventListener('submit',async event=>{event.preventDefault();const licenseForm=event.currentTarget;const result=await api('/api/licenses',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({agency:'ODWC',licenseName:document.querySelector('#license-name').value,species:document.querySelector('#license-species').value,licenseNumberMasked:document.querySelector('#license-number').value,expiresOn:document.querySelector('#license-expiry').value,captureMode:localLicenseScreenshot?'screenshot-reviewed-local':'manual-entry'})});if(!result.response.ok){setAccountMessage(result.data?.error||'Sign in before saving a license snapshot.');return;}if(localLicenseObjectUrl){URL.revokeObjectURL(localLicenseObjectUrl);localLicenseObjectUrl=null;}setAccountMessage(localLicenseScreenshot?'Confirmed screenshot fields saved; raw image was not uploaded.':'License snapshot saved.',true);licenseForm.reset();document.querySelector('#license-preview-image').removeAttribute('src');document.querySelector('#license-preview').hidden=true;licenseForm.hidden=true;localLicenseScreenshot=false;await loadAccount();});
loadAccount();
